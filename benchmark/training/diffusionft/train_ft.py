...
from util import IterExponential, replace_unet_conv_in, add_marker_embedding_to_unet, pixel_mix_loss
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to pretrained model unet model",
    )
    parser.add_argument(
        "--lr_exp_warmup_steps",
        type=int,
...
def main():
    ...
    if 0. < args.lambda_pixel_mix_loss < 1.:
        raise ValueError("lambda_pixel_mix_loss should be between 0. and 1.")
    ...
    unet         = UNet2DConditionModel.from_pretrained(args.pretrained_unet_checkpoint, subfolder="unet", revision=None)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            for _ in range(len(models)):
                ...
                add_marker_embedding_to_unet(load_model, train_dataset.num_channels)
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            accelerator.load_state(os.path.join(args.output_dir, path), load_kwargs={"weights_only": False})
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):     
                ...
                ground_truth = batch["target"].to(device=accelerator.device, dtype=weight_dtype)

                loss = pixel_mix_loss(
                    current_estimate.float(), ground_truth.float(), lam=args.lambda_pixel_mix_loss, reduction="mean")
