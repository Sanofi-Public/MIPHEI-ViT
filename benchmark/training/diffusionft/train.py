...
from util import IterExponential, replace_unet_conv_in, add_marker_embedding_to_unet
from dataset import ORIONDataset
...
def stack_gt_images(gt_in):
    if 4 == len(gt_in.shape):
        stacked = gt_in.repeat(1, 3, 1, 1)
    elif 3 == len(gt_in.shape):
        stacked = gt_in.unsqueeze(1).repeat(1, 3, 1, 1)
    return stacked

def encode_gt(vae, gt):
    # stack depth into 3-channel
    stacked = stack_gt_images(gt)
    # encode using VAE encoder
    latent = encode_image(vae, stacked)
    return latent
...
def main():
    ...
    # Training datasets
    orion_root_dir = "/root/workdir/ORION_dataset_20x" ##############
    train_dataset = ORIONDataset(data_dir=orion_root_dir, split="train")
    ...
    add_marker_embedding_to_unet(unet, train_dataset.num_channels)
    assert unet.config.class_embed_type == "projection"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):     
                ...
                gt_latents = encode_gt(vae, batch["target"].to(device=accelerator.device, dtype=weight_dtype))
                gt_latents = gt_latents * vae.config.scaling_factor

                marker_id = batch["marker_id"].to(accelerator.device, dtype=torch.long)
                marker_id_onehot = F.one_hot(marker_id, num_classes=train_dataset.num_channels).to(weight_dtype)
                marker_embed = torch.cat([torch.sin(marker_id_onehot), torch.cos(marker_id_onehot)], dim=-1)

                # Set timesteps to the first denoising step
                timesteps = torch.randint(
                    0, noise_scheduler.config.num_train_timesteps, (rgb_latents.shape[0],), device=rgb_latents.device
                )
                timesteps = timesteps.long()
                
                # Sample noisy latent
                noise = torch.randn_like(gt_latents).to(accelerator.device)
                noisy_latents = noise_scheduler.add_noise(gt_latents, noise, timesteps)

                # Generate UNet prediction
                encoder_hidden_states = empty_encoding.repeat(len(batch["rgb"]), 1, 1)
                unet_input = torch.cat((rgb_latents, noisy_latents), dim=1).to(accelerator.device)
                model_pred = unet(unet_input, timesteps, encoder_hidden_states,
                                  class_labels=marker_embed, return_dict=False)[0]

                if noise_scheduler.config.prediction_type == "epsilon":
                    target = noise
                elif noise_scheduler.config.prediction_type == "v_prediction":
                    # v = α_t * ε − σ_t * x0 ; we can get target via scheduler
